...
> Wails v2 桌面应用模板 + Claude Skill。Go + Vue 3 + Naive UI，提炼自多个生产项目，开箱即用。
...
- ✅ **Claude Skill**：AI 辅助初始化新项目，自动替换占位符、裁剪可选模块

## 快速使用（30 秒新建项目）

### 方式 1：用 Claude Skill（推荐）
...
Claude 会问你几个问题，然后自动初始化好骨架。

### 方式 2：用初始化脚本

```bash
# 1. 复制模板
cp -R /Users/curie/GoProject/AnchorHub-wails/template ~/GoProject/MyTool
cd ~/GoProject/MyTool

# 2. 交互式初始化（替换占位符）
bash /Users/curie/GoProject/AnchorHub-wails/scripts/init-project.sh
